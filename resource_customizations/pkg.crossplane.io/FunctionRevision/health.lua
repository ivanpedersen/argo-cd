...
if obj.status ~= nil and obj.status.conditions ~= nil then
  ...
  for i, condition in ipairs(obj.status.conditions) do
    ...
    if condition.type == "Healthy" then
      if condition.status == "True" then
        hs.status = "Healthy"
        hs.message = condition.reason or "Function revision is healthy"
        return hs
      elseif condition.status == "False" then
        hs.status = "Degraded"
        hs.message = condition.message or condition.reason or "Function revision is unhealthy"
        return hs
      end
    ...
    elseif condition.type == "RevisionHealthy" then
      ...
      elseif condition.status == "False" then
        degradedMsg = condition.message or condition.reason or "Function revision is unhealthy"
      end
    elseif condition.type == "RuntimeHealthy" then
      if condition.status == "True" then
        runtimeHealthy = true
      elseif condition.status == "False" then
        degradedMsg = condition.message or condition.reason or "Function runtime is unhealthy"
      end
    end
  end
  ...
  if revisionHealthy and runtimeHealthy then
    hs.status = "Healthy"
    hs.message = "Function revision is healthy"
    return hs
  end
end
